/* global api, app, ajaxify, alerts, components */

const toggleFavorite = async (button, tid) => {
	const userFavoritesResponse = await api.get(
		`/plugins/users/favorites/${app.user.uid}`,
	);
	...
	const newFavorites = isFavorited ? userFavorites.filter(id => id !== tid) : [...userFavorites, tid];

	api.post(
		`/plugins/users/favorites/${app.user.uid}`,
		{ uid: app.user.uid, favorites: newFavorites.join(',') },
		(err) => {
			if (err) {
				if (!app.user.uid) {
					ajaxify.go('login');
...
const initFavoriteButton = (button) => {
	...
	api
		.get(`/plugins/users/favorites/${app.user.uid}`)
		.then((userFavoritesResponse) => {
			const userFavorites = userFavoritesResponse
				.split(',')
				.map(Number)
				.filter(num => !isNaN(num));
